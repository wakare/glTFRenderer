...
- ZH: 当前 `LightmapPathTracingPass` 已经具备 factor 材质、`baseColorTexture`、`normalTexture`、`emissiveTexture` 和首版 punctual direct lighting 的最小 diffuse path tracing 能力，后续继续补更完整的 alpha mask / metallic-roughness 材质路径和更高质量采样策略。
- EN: The current `LightmapPathTracingPass` already supports a minimum diffuse path tracing path with factor materials, `baseColorTexture`, `normalTexture`, `emissiveTexture`, and a first-pass punctual direct-lighting path; the next work is fuller alpha-mask / metallic-roughness material support and higher-quality sampling strategies.
