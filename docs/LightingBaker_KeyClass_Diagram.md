...
- ZH: 当前 `LightmapPathTracingPass` 已经具备 factor 材质、`baseColorTexture`、`metallicRoughnessTexture`、`normalTexture`、`emissiveTexture`、alpha-masked 可见性，以及带 `--direct-light-samples` 控制、luminance-weighted light selection 的 punctual direct lighting 采样路径；该路径现已对 point / spot / directional 三类 punctual light 建立回归验证。同一套 next-event estimation 预算现在也接入了 emissive triangle direct sampling，并在 emissive direct samples 与 BSDF continuation hits 之间应用 PDF-based MIS，同时支持用 `--direct-light-samples 0` 显式关闭 NEE；现有回归验证表明，“N bounce + emissive direct sampling” 近似对齐 “N+1 bounce emissive baseline”。同时，sky 环境项已经升级为带 `--environment-light-samples` 控制的 true sky importance sampling，并对 direct environment sampling 与 BSDF miss path 使用 PDF-based MIS；现有回归验证了 “N bounce + environment direct sampling” 近似对齐 “N+1 bounce baseline”。alpha 回归矩阵也已经扩展到 `opaque / factor_mask / factor_cutoff_boundary / texture_mask / texture_cutoff_boundary / blend_texture_fallback`。后续重点收敛到更完整的 BSDF / light MIS，尤其是 punctual / emissive / environment / BSDF 之间的组合，以及运行时导向的压缩 codec。
- EN: The current `LightmapPathTracingPass` already supports factor materials, `baseColorTexture`, `metallicRoughnessTexture`, `normalTexture`, `emissiveTexture`, alpha-masked visibility, and a punctual direct-light sampling path controlled by `--direct-light-samples` with luminance-weighted light selection; that path is now regression-validated across point / spot / directional punctual lights. The same next-event estimation budget now also drives emissive-triangle direct sampling, with PDF-based MIS between emissive direct samples and BSDF continuation hits, and `--direct-light-samples 0` explicitly disables NEE; regression coverage now shows that “N bounce + emissive direct sampling” tracks the corresponding “N+1 bounce emissive baseline”. The sky environment term has now moved to true sky importance sampling controlled by `--environment-light-samples`, with PDF-based MIS between direct environment samples and the BSDF miss path. Regression coverage also shows that “N bounce + environment direct sampling” tracks the corresponding “N+1 bounce baseline”. The alpha regression matrix now also covers `opaque / factor_mask / factor_cutoff_boundary / texture_mask / texture_cutoff_boundary / blend_texture_fallback`. The next work is fuller BSDF / light MIS, especially across punctual / emissive / environment / BSDF combinations, plus runtime-oriented compression codecs.
