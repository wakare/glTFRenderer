...
- ZH: 当前 `LightmapPathTracingPass` 已经具备 factor 材质和 `baseColorTexture` 的最小 diffuse path tracing 能力，后续继续补 direct lighting、更多材质输入和更高质量采样策略。
- EN: The current `LightmapPathTracingPass` already supports a minimum diffuse path tracing path with factor materials and `baseColorTexture`; the next work is direct lighting, broader material inputs, and higher-quality sampling strategies.
