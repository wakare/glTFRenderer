...
当前已落地到可运行状态的是“`--resume` 校验 + progressive 继续累积的 DXR atlas-domain diffuse path tracing`”：
...
当前已经不再依赖 `debug hemisphere` 占位积分器；现阶段的最小求解器已经是可运行的 DXR atlas-domain diffuse path tracing，并已接入 factor 材质、`baseColorTexture` 和 `emissiveTexture`。仍未覆盖的内容包括 normal map、metallic-roughness 贴图、alpha mask 可见性、显式 direct light sampling 和更高阶的重要性采样策略。
...
- 当前 atlas texel 域 DXR diffuse path tracing 已经落地，并已接入 `baseColorTexture` / `emissiveTexture`；后续继续补 alpha mask、direct lighting、sampling 策略和更完整的 path payload
...
从现有代码基础看，`LightingBaker` 已经从“纯规划”进入“可运行 DXR 原型”阶段：独立工程骨架、UV1/lightmap 绑定通路、atlas texel records、sidecar writer、runtime importer、`--resume` 校验，以及 atlas texel 域的 progressive DXR diffuse path tracing 都已经打通。
...
现在这条 progressive 管线的价值，已经不只是固定 cache / package / runtime contract，而是作为真正的 DXR lightmap baker 主链继续迭代。后续新增 alpha mask、direct lighting、更多材质输入和压缩 codec，都会在这条已打通的 atlas-domain 路径上逐步扩展。
