...
The baker no longer depends on the `debug hemisphere` placeholder integrator. The current minimum solver is already runnable DXR atlas-domain diffuse path tracing with factor materials, `baseColorTexture`, `normalTexture`, `emissiveTexture`, and a first-pass punctual direct-lighting path. Remaining gaps include metallic-roughness textures, more systematic alpha-mask regression coverage, more advanced importance sampling, and fuller lighting strategies beyond the current punctual-light path.
...
The value of the current progressive pipeline is no longer limited to locking down the cache / package / runtime contract. It is now the live DXR lightmap-baker path that future work will extend with metallic-roughness and fuller alpha-mask material visibility, fuller direct-light / importance sampling, and runtime-oriented compression.
