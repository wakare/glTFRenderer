...
The baker no longer depends on the `debug hemisphere` placeholder integrator. The current minimum solver is already runnable DXR atlas-domain diffuse path tracing with factor materials, `baseColorTexture`, `metallicRoughnessTexture`, `normalTexture`, `emissiveTexture`, alpha-masked visibility, and a punctual direct-light sampling path controlled by `--direct-light-samples` with luminance-weighted light selection. That path is now regression-validated across point / spot / directional punctual lights. The same next-event estimation budget now also drives emissive-triangle direct sampling, with PDF-based MIS between emissive direct samples and BSDF continuation hits, and `--direct-light-samples 0` explicitly disables NEE; regression coverage now shows that “N bounce + emissive direct sampling” tracks the corresponding “N+1 bounce emissive baseline”. The sky environment term has also moved to true sky importance sampling controlled by `--environment-light-samples`, with PDF-based MIS between direct environment samples and the BSDF miss path. Regression coverage also shows that “N bounce + environment direct sampling” tracks the corresponding “N+1 bounce baseline”. The alpha regression matrix also now covers `opaque / factor_mask / factor_cutoff_boundary / texture_mask / texture_cutoff_boundary / blend_texture_fallback`. The next gaps are fuller BSDF / light MIS, especially across punctual / emissive / environment / BSDF combinations, plus runtime-oriented compression.
...
- the atlas-domain DXR diffuse path tracing pass is now live; it already includes regression-backed punctual direct lighting, emissive-triangle direct sampling with PDF-based MIS, true sky importance sampling, and the expanded alpha matrix, while the next work is fuller BSDF / light MIS and export/runtime quality
...
The value of the current progressive pipeline is no longer limited to locking down the cache / package / runtime contract. It is now the live DXR lightmap-baker path, and it already includes configurable, regression-validated punctual direct-light sampling with luminance-weighted light selection plus baseline coverage across point / spot / directional punctual lights. The same `--direct-light-samples` budget now also covers emissive-triangle direct sampling, with `0` explicitly disabling NEE and PDF-based MIS between emissive direct samples and BSDF continuation hits; formal regression coverage shows that “N bounce + emissive direct sampling” tracks the corresponding “N+1 bounce emissive baseline”. The sky environment path now also has formal `--environment-light-samples` true sky importance sampling with PDF-based MIS and regression coverage, while the alpha path now has a formal matrix covering `opaque / factor_mask / factor_cutoff_boundary / texture_mask / texture_cutoff_boundary / blend_texture_fallback`. Future work should extend it with fuller BSDF / light MIS and runtime-oriented compression.
