...
public:
  ...
  RendererSceneAABB(const glm::vec3& center, float radius);

  /// Builds an AABB that contains the two points.
  RendererSceneAABB(const glm::vec3& p1, const glm::vec3& p2);

  RendererSceneAABB(const RendererSceneAABB& aabb);
  ~RendererSceneAABB();
  ...
  void extend(const RendererSceneAABB& aabb);
  ...
  /// \xxx Untested -- This function is not represented in our unit tests.
  bool overlaps(const RendererSceneAABB& bb) const;
  ...
  INTERSECTION_TYPE intersect(const RendererSceneAABB& bb) const;
  ...
  /// \xxx Untested -- This function is not represented in our unit tests.
  bool isSimilarTo(const RendererSceneAABB& b, float diff = 0.5) const;

  static RendererSceneAABB TransformAABB(const glm::mat4& mat, const RendererSceneAABB& input );
