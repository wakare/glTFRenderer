...
std::shared_ptr<IRHIVertexBufferView> DX12VertexBuffer::CreateVertexBufferView(IRHIDevice& device,
                                                                               glTFRenderResourceManager& resource_manager, IRHICommandList& command_list, const RHIBufferDesc& desc, const VertexBufferData& vertex_buffer_data)
{
    m_vertex_layout = vertex_buffer_data.layout;
    m_vertex_count = vertex_buffer_data.vertex_count;
    
    resource_manager.GetMemoryManager().AllocateBufferMemory(device, desc, m_buffer);
    ...
    resource_manager.GetMemoryManager().AllocateBufferMemory(device, vertex_upload_buffer_desc, m_upload_buffer);
