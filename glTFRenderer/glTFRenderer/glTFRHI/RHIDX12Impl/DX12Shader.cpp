...
bool DX12Shader::CompileShaderWithDXC()
{
    ...
    //compile_argument_strings.emplace_back(L"D:\\Work\\DevSpace\\glTFRenderer\\glTFRenderer\\glTFRenderer\\x64\\Debug\\");     // The file name of the pdb. This must either be supplied or the autogenerated file name must be used.
    ...
    if (!m_macros.macroKey.empty())
    {
        for (size_t i = 0; i < m_macros.macroKey.size(); i++)
        {
            ...
            LOG_FORMAT_FLUSH("[DEBUG] Compile Compile shader file %s with macro %s\n", m_shader_file_path.c_str(), define_string)
