...
bool DX12GraphicsPipelineStateObject::InitPipelineStateObject(IRHIDevice& device, const RHIPipelineStateInfo& pipeline_state_info)
{
    ...
    {
        dx_input_layouts.push_back({input_layout.semantic_name.c_str(), input_layout.semantic_index, DX12ConverterUtils::ConvertToDXGIFormat(input_layout.format),
            input_layout.slot, input_layout.aligned_byte_offset, input_layout.IsVertexData() ? D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA : D3D12_INPUT_CLASSIFICATION_PER_INSTANCE_DATA, static_cast<unsigned>(input_layout.IsVertexData() ? 0 : 1)});  
    }
