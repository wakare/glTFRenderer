...
std::shared_ptr<IRHIVertexBufferView> VKVertexBuffer::CreateVertexBufferView(IRHIDevice& device, IRHIMemoryManager& memory_manager,
                                                                             IRHICommandList& command_list, const RHIBufferDesc& desc, const VertexBufferData& vertex_buffer_data)
{
    ...
    RHIUtils::Instance().CopyBuffer(command_list, *m_buffer->m_buffer, 0, *m_upload_buffer->m_buffer, 0, vertex_buffer_data.byte_size);
